...
class ListMethods:
    def __init__(self,l):
        ...
        try:
            if type(l) == list:
                self.l = l
                logging.info(f"List object is created with value : {l}")
            else:
    ...
    def count_list(self,x):
        """
        This method will return number of occurances of parameters passed.
        """
        try :
            c = 0
            for i in self.l :
                if x == i:
                    c += 1
            logging.info(f"number of occurances of parameter {x} in the list : {c}")
            return c
        except Exception as e:
            print(e)
            logging.exception(str(e))
    ...
    def index_list(self,x):
        """ This method will return the parameter of first index"""
        try :
    ...
    def remove_list(self,x):
        """ This method will remove the fist occurance of parameter"""
        try:
            c = self.index_list(x)
            if type(x) == int:
                x =self.pop_list(c)
